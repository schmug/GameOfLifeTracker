...
export default function GameGrid({
  ...
  const updateStats = useCallback(() => {
    ...
    const calculatedDensity = Math.round(
      (livingCount / (gridSize * gridSize)) * 100,
    );
  ...
  // Load a predefined pattern centered on the grid
  const loadPattern = useCallback(
    (coords: [number, number][]) => {
      const empty = createEmptyGrid(gridSize);
      const maxRow = Math.max(...coords.map((c) => c[0]), 0);
      const maxCol = Math.max(...coords.map((c) => c[1]), 0);
      const startRow = Math.floor((gridSize - (maxRow + 1)) / 2);
      const startCol = Math.floor((gridSize - (maxCol + 1)) / 2);
      coords.forEach(([r, c]) => {
        const row = startRow + r;
        const col = startCol + c;
        if (row >= 0 && row < gridSize && col >= 0 && col < gridSize) {
          empty[row][col] = { alive: true, color: getRandomColor() };
        }
      });
      setGrid(empty);
  ...
  const toggleCell = useCallback(
    (rowIndex: number, colIndex: number, forcedState?: boolean) => {
      ...
      if (newAlive && !cell.alive) {
        ...
        try {
          const cellElement = document.querySelector(
            `.game-grid > div:nth-child(${rowIndex * gridSize + colIndex + 1})`,
          );
          if (cellElement) {
  ...
  }, [clearGrid, randomizeGrid, stepForward, loadPattern]);

  return (
    <div
      className={`relative border border-grid rounded-lg shadow-sm bg-white overflow-hidden ${showGameOverOverlay ? "game-over-overlay-active" : ""}`}
    >
